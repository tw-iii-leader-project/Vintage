...
public class LeaveMsgServiceImp implements LeaveMsgService {
	...
	private LeaveMsgEntity setLeaveMsgEntity(MsgAddReq req) {
		...
		leaveMsgEntity.setEmail(req.getEmail());
		leaveMsgEntity.setUserName(req.getUserName());
		leaveMsgEntity.setLeaveMsgContent(req.getLeaveMsgContent());
		leaveMsgEntity.setLeaveMsgTime(Timestamp.valueOf(LocalDateTime.now()));
		leaveMsgEntity.setUserPic(req.getUserPic());
		leaveMsgEntity.setPId(req.getPId());
	...
	public ShowPorductLeaveMsgResp showLeaveMsg(ShowPorductLeaveMsgReq productId) {
		List<LeaveMsgEntity> listLeaveMsgEntity = leaveMsgRepository.findByPId(productId.getPId());
